...
import argparse
import torch
from models.st_gcn.st_gcn import STGCNEmbedding, GraphTransformerEmbedding
...
def get_model_GraphTransformer(opt):

    graph_args = {"strategy": "uniform"}

    embedding_net = GraphTransformerEmbedding(
        node_in_dim=3,
        hidden_dim=opt.embedding_layer_size,
        num_gt_layers=opt.layers,
        heads = opt.heads
    )
    ...
    return embedding_net, model_args
...
def get_trainer(model, opt, steps_per_epoch):
    optimizer = torch.optim.Adam(
        model.parameters(), lr=opt.learning_rate, weight_decay=opt.weight_decay, eps = 1e-6
    )

...
import wandb
wandb.login(key = "7b758815ae733f76b67a2a856683cd4080ebe91f")
...
def main(opt):
    ...
    if opt.network_name=='gt':
        model, model_args = get_model_GraphTransformer(opt)
    elif opt.network_name in ['resgcn-n39-r8', 'resgcn_transformer-n39-r8']:
        model, model_args = get_model_resgcn(graph, opt)
    elif opt.network_name=='get_model_resgcn_transformer':
        model, model_args = get_model_resgcn(graph, opt)
